/*
 * What one customer pays for one product, and which rules decided it — and which did NOT.
 *
 ...
 * be tried without an order to hang it on. The "did not apply" list is the most realism
 * per line on the screen: "not valid until 1 Oct", "minimum quantity 10, this line is 4".
 *
 * It sits under the rules rather than beside them: the fields are short and read
...
/** The quote in a sentence, then the rules that were tried and set aside, each with why. */
function Result ({ quote, nameFor }) {
  ...
  return (
    <>
      ...
      {line.notApplied && line.notApplied.length > 0 && (
        <View marginTop='size-150'>
          <Text UNSAFE_className='erp-subtle'>Did not apply:</Text>
          <ul className='erp-plain-list'>
            {line.notApplied.map((r) => (
              <li key={r.id}><Text>{`${ruleOf(r.kind).label} (${ruleOf(r.kind).code}) — ${r.reason}`}</Text></li>
            ))}
          </ul>
        </View>
      )}
  ...
  return (
    <View marginTop='size-500'>
      ...
      <div className='erp-inline-form'>
        <div style={{ width: 280 }}>
          <RecordPicker label='Customer' items={customerItems(customers || [])} selectedKey={input.partnerId} onChange={(partnerId) => set({ partnerId })} />
        </div>
        <div style={{ width: 300 }}>
          <RecordPicker label='Product' items={productItems(products)} selectedKey={input.sku} onChange={(sku) => set({ sku })} isRequired />
        </div>
