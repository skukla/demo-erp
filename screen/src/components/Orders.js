...
export default function Orders ({ api, onChanged }) {
  ...
  return (
    <Frame title='Sales orders' error={moveError || error} loading={!rows}>
      <TableView {...widths.tableProps} aria-label='Sales orders' density='compact' overflowMode='wrap'>
        <TableHeader>
          <Column key='number' {...widths.columnProps('number')}>Sales order</Column>
          <Column key='commerce' {...widths.columnProps('commerce')}>Commerce order</Column>
          <Column key='partner' {...widths.columnProps('partner')}>Partner</Column>
          <Column key='lines' {...widths.columnProps('lines')} align='end'>Lines</Column>
