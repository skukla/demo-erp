...
async function search (cols, q) {
  ...
  for (const o of orders) {
    add(score([o.number, o.commerceIncrementId, o.commerceOrderId, names.get(o.partnerId)], needle), {
      kind: 'order', number: o.number, title: `Sales Order ${o.number}`, subtitle: [names.get(o.partnerId), o.commerceIncrementId ? `Ref ${o.commerceIncrementId}` : null].filter(Boolean).join(' · ')
    })
  }
