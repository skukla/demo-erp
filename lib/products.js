...
/* Sellable, or blocked for sales: SAP's distribution-chain sales status (X-plant material
   status), Business Central's Blocked item. The ERP's own decision — Commerce is not told,
   and an import never resets it — and a blocked product ships nothing (lib/fulfilment). */
...
async function importProducts (cols, rows) {
  ...
  for (const row of rows) {
    ...
    const product = stored({
      ...
      // Commerce does not carry a sales status; the ERP's own stands across imports.
 ...
 * changed name or price raises one product update; changed quantities raise one stock
 * update naming each source. Subscribers carry both to Commerce. The sales status raises
 * nothing: it is the ERP's own. The SKU is the link to Commerce and cannot change here.
 *
...
/**
 * Remove a product the way Commerce removed it. Deleting a configurable product in
 * Commerce leaves its children as products of their own, so a parent's variants stay
