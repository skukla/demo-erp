...
const { daysOf, dueDate } = require('./terms')
...
async function describeInvoice (cols, order, invoice) {
  ...
  return {
    ...
    // When it is due: the billing date plus the sold-to's payment terms (lib/terms); null
    // when the terms name no number of days.
    paymentDays: daysOf(partner && partner.paymentTerms),
    dueDate: dueDate(invoice.createdAt, partner && partner.paymentTerms),
