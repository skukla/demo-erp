...
function otherSalesOrg (condition, salesOrg) {
  if (condition.salesOrg && salesOrg && condition.salesOrg !== salesOrg) return `for sales organisation ${condition.salesOrg}; this is ${salesOrg}`
  return null
}
 ...
 *
 * @param {object} args `{ product, partner, conditions, qty, date, salesOrg }`
 * @returns {object} `{ sku, qty, listPrice, contractPrice, discountPercent, maxDiscountPercent, source }`
 */
function priceLine ({ product, partner, conditions, qty = 1, date = today(), salesOrg }) {
  ...
  const notApplied = conditions
    ...
    .map((c) => ({ id: c._id, kind: c.kind, reason: ruledOut(c, context) || otherSalesOrg(c, salesOrg) || outranked(c, { price, discount, ceiling }) }))
    .filter((c) => c.reason)
 ...
 */
function quote ({ products, partner, conditions, lines, date = today(), salesOrg }) {
  ...
  for (const line of lines) {
    ...
    priced.push(priceLine({ product, partner, conditions, qty: line.qty ?? 1, date, salesOrg }))
  }
