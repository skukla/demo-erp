 ...
 *   { kind: 'contractPrice', partnerId, sku, price }              a partner's price for a material
 *   { kind: 'contractDiscount', partnerId, sku?: null, percent }  a partner's discount, all materials when sku is null
...
function priceLine ({ material, partner, conditions, qty = 1 }) {
  ...
  return { sku, qty, listPrice, contractPrice: round2(contractPrice), discountPercent, maxDiscountPercent, source, lineTotal: round2(contractPrice * qty) }
}
 ...
 */
function quote ({ materials, partner, conditions, lines }) {
  const bySku = new Map(materials.map((m) => [m.sku, m]))
  const priced = []
  for (const line of lines) {
    const material = bySku.get(line.sku)
    if (!material) {
      priced.push({ sku: line.sku, qty: line.qty ?? 1, unknown: true })
      continue
    }
    priced.push(priceLine({ material, partner, conditions, qty: line.qty ?? 1 }))
  }
  const total = round2(priced.reduce((sum, l) => sum + (l.lineTotal || 0), 0))
  return { partnerId: partner ? partner.id : null, lines: priced, total }
}

module.exports = { DEFAULT_MAX_DISCOUNT, specificity, matches, mostSpecific, round2, priceLine, quote }
