...
const { resolvePartner, exposureOf, widenSalesOrgs } = require('./partners')
...
const CANCEL_REASONS = [
  ...
  'Pricing error',
  'Duplicate order',
  // The one reason the ERP does not choose: Commerce cancelled the order and said so.
  'Cancelled in Commerce'
]
...
function abilities (order) {
  ...
  const held = order.creditStatus === 'held' && order.header !== 'cancelled'
  return {
 ...
 * @param {object} input `{ commerceOrderId, commerceIncrementId?, partnerId?, commerceCompanyId?, email?, customerGroupId?, lines:[{sku, qty, price}], currency?, total?, salesOrg?, salesOrgName? }`
...
async function createOrder (cols, input, params) {
  ...
  // (plan §6.1: the order already exists in Commerce, and a refusal would drop it).
  const net = cents(lines.reduce((s, l) => s + l.qty * l.price, 0))
  const exposure = partner ? await exposureOf(cols, partner.id) : 0
  const credit = decide({ partner, exposure, net })
  ...
  // Commerce hears the hold at once, so its order shows On Hold while the ERP's does —
  // SAP's blocked sales document, seen from the shop's side.
  if (credit.status === 'held') await emit(cols, 'order.hold', { ...orderEventPayload(order), held: true, reason: credit.reason }, params)
  return view(order)
}
...
async function describeOrder (cols, stored) {
  ...
  /* The warehouses this order's products are stocked in: the Ship-from choices, under
     the ERP's own names (settings) with the Commerce name beside. */
  ...
  for (const product of products.values()) {
    for (const w of product.warehouses || []) {
      ...
      warehouses.set(w.code, { code: w.code, name: own || w.name, commerceName: w.name })
    }
  }
