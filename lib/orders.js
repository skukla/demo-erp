 ...
 * through their statuses from the ERP's screen, each move an outbox entry.
 */
 ...
 * @param {object} input `{ commerceOrderId, commerceIncrementId?, partnerId, lines:[{sku, qty, price}], currency?, total? }`
...
async function createOrder (cols, input) {
  ...
  const lines = (input.lines || []).map((l) => ({ sku: l.sku, qty: Number(l.qty) || 1, price: Number(l.price) || 0 }))
  const order = {
    _id: number,
    number,
...
/**
 * Move an order to a new status. Refuses a move the machine does not allow; emits
 * an outbox entry the integration turns into the Commerce operation for that status.
 *
 * @returns {Promise<object>} the order after the move
 */
async function setStatus (cols, number, status) {
  ...
  await emit(cols, { kind: 'order.status', number, commerceOrderId: order.commerceOrderId, commerceIncrementId: order.commerceIncrementId, status })
  return next
}

module.exports = { STATUSES, TRANSITIONS, nextStatuses, createOrder, listOrders, getOrder, setStatus }
