...
const EVENT_NAMES = {
  ...
  // One event for both directions of a credit hold: `held` true puts the Commerce order On
  // Hold, false takes it off (a release). A reject is a cancel and travels as one.
  'order.hold': 'be-observer.sales_order_hold',
...
async function emit (cols, kind, value, params, deps) {
  ...
  const entry = { _id: randomUUID(), at: new Date().toISOString(), direction: 'out', kind, event, value, delivered: false, failed: false, attempts: 0 }
...
/**
 * Journal a change that arrived FROM Commerce, so the log shows both directions.
 *
 * Only outbound entries were journaled until 2026-09-18, so a change Commerce sent
 ...
 *
 * @param {object} cols collections
 * @param {string} event the Commerce event (or "Sync from Commerce") that brought it
 * @param {string} summary what the ERP did with it, in words
 * @param {object} [value] the ids involved
 * @param {string} [eventId] the delivered event's own id, when the integration passed it
 * @returns {Promise<object>} the journal entry
 */
async function receive (cols, event, summary, value = {}, eventId) {
  const entry = { _id: randomUUID(), at: new Date().toISOString(), direction: 'in', event, summary, value, ...(eventId ? { eventId } : {}) }
  await cols.events.replaceOne({ _id: entry._id }, entry, { upsert: true })
  return entry
}
...
module.exports = { EVENT_NAMES, MAX_ATTEMPTS, webhookUrl, authHeaders, deliver, emit, receive, pending, failed, recent, retryPending, requeueFailed }
