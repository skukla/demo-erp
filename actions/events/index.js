/* GET events: the newest journal entries; POST events/retry: redeliver what is pending; POST events/requeue: give failed events another ten attempts. */
...
async function handler ({ cols, method, segments, params }) {
  if (method === 'GET') {
    return ok({ webhookUrl: webhookUrl(params), pending: (await pending(cols)).length, failed: (await failed(cols)).length, items: await recent(cols) })
  }
